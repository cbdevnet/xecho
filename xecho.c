...
void draw_string(char* text, char* font_name, int width, int height, int border, Display* display, XftDraw* drawable, XftColor c){
	...
	XftColor color=c;
	XftDrawString8(drawable, &color, font, /*((width-extents.width)/2)*/0, ((height-extents.height)/2)+extents.height/*height*/, text, strlen(text)); 
//	XftDrawRect(drawable, &color, 0, height, extents.width, extents.height); 
	XftFontClose(display, font);
}
